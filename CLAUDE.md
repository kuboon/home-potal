...
編集・author/admin 削除）+
**Repost**（任意の投稿を引用してスレッドへピックアップ、 リンク平坦化）+
**レート制限**（投稿 1/秒・20/分、Repost 5/分、Deno KV 固定ウィンドウ）+
...
Bearer トークンを発行。Home には id 指定で メンバー追加）まで実装済み。後続で
MCP サーバ（エージェントがトークンで Web UI 同等の ツールを実行）を積み上げる。
